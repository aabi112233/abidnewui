...
export default function AdminPaymentAccountsPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input value={form.accountNumber || ""} onChange={e => setForm(p => ({ ...p, accountNumber: e.target.value }))}
                placeholder="e.g. 0300-1234567 or PK12MEZN0001234567890101" className="elegant-input text-sm font-mono" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-[var(--text-primary)] flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-[var(--brand-600)]" /> Payment Accounts
          </h2>
          <p className="text-[var(--text-secondary)] text-sm mt-0.5 font-medium">These appear on the Register & Store pages for students to pay.</p>
        </div>
      ...
      {msg && <div className={`p-3 rounded-xl text-sm font-bold ${msg.startsWith("✅") || msg.startsWith("🗑️") ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}>{msg}</div>}

      {loading ? (
        <div className="elegant-card p-12 text-center text-[var(--text-tertiary)]">Loading...</div>
      ) : accounts.length === 0 ? (
        <div className="elegant-card p-12 text-center border-dashed">
          ...
          <h3 className="text-xl font-bold text-[var(--text-secondary)] mb-2">No Payment Accounts Yet</h3>
            ...
            <div key={acc.id} className={`elegant-card p-5 flex flex-col sm:flex-row sm:items-center gap-4 animate-fade-up transition-opacity ${!acc.isActive ? "opacity-50" : ""}`} style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border ${TYPE_COLORS[acc.type] || "bg-slate-50 text-slate-700 border-slate-200"}`}>{acc.type}</span>
                  {!acc.isActive && <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-red-50 text-red-500 border border-red-100">Hidden</span>}
                </div>
                <div className="text-sm text-[var(--text-secondary)] font-medium">{acc.accountTitle}</div>
                <div className="flex items-center gap-1">
                  <span className="font-mono text-sm text-[var(--text-primary)] bg-[var(--bg-main)] border border-[var(--border-strong)] px-2 py-0.5 rounded-lg">{acc.accountNumber}</span>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button title={acc.isActive ? "Hide" : "Show"} onClick={() => toggleActive(acc)}
                  className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-colors ${acc.isActive ? "bg-green-50 text-green-600 border-green-200 hover:bg-green-100" : "bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200"}`}>
                  {acc.isActive ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                </button>
                <button title="Edit" onClick={() => openEdit(acc)} className="w-9 h-9 rounded-xl bg-[var(--brand-50)] hover:bg-[var(--brand-100)] text-[var(--brand-600)] flex items-center justify-center border border-[var(--brand-100)] transition-colors">
                  <Pencil className="w-4 h-4" />
                </button>
                <button title="Delete" onClick={() => setConfirmDel(acc.id)} className="w-9 h-9 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 flex items-center justify-center border border-red-100 transition-colors">
